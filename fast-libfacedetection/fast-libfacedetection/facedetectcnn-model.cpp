...
void init_parameters()
{
    ...
    {
        int i = 0, ff = 0;
        param_pFilters[i].stride = 1;
        param_pFilters[i].pad = 0;
		...
		param_pFilters[i].filters = (CDataBlob *)calloc(sizeof(CDataBlob), param_pConvInfo[i].num);

		CDataBlob b3x3 = { 0 };

        for(ff = 0; ff < param_pConvInfo[i].num; ++ff)
        {
			CDataBlobInit2(&b3x3, param_pConvInfo[i].width, param_pConvInfo[i].height, param_pConvInfo[i].channels);
#if defined(_ENABLE_INT8_CONV)
...
#else
			setFloatDataFromCaffeFormat(&b3x3, param_ppConvCoefFloat[i] + ff * offset,
				param_pConvInfo[i].width, param_pConvInfo[i].height, param_pConvInfo[i].channels);
            blob2vector(&b3x3, &param_pFilters[i].filters[param_pFilters[i].tail], true);
#endif
			param_pFilters[i].tail++;
			CDataBlobDeinit(&b3x3);
        }
    }
    //set the rest
	int i, ff;
    for(i = 1; i < NUM_CONV_LAYER; ++i)
    {
        ...
        for(ff = 0; ff < param_pConvInfo[i].num; ++ff)
        {
			CDataBlobInit2(&param_pFilters[i].filters[param_pFilters[i].tail], param_pConvInfo[i].width, param_pConvInfo[i].height, param_pConvInfo[i].channels);
#if defined(_ENABLE_INT8_CONV)
            b->setInt8DataFromCaffeFormat(param_ppConvCoefInt8[i] + ff * offset, 
                                            param_pConvInfo[i].width, param_pConvInfo[i].height, param_pConvInfo[i].channels);
#else
			setFloatDataFromCaffeFormat(&param_pFilters[i].filters[param_pFilters[i].tail], param_ppConvCoefFloat[i] + ff * offset,
                                            param_pConvInfo[i].width, param_pConvInfo[i].height, param_pConvInfo[i].channels);
#endif
			param_pFilters[i].tail++;
        }
...
cvector objectdetect_cnn(unsigned char * rgbImageData,
	int width,
	int height,
	int step)
{
	CDataBlob inputImage = { 0 };
	CDataBlobInit1(&inputImage);
	CDataBlob pConvDataBlobs[NUM_CONV_LAYER] = { 0 };
	CDataBlob pool1 = { 0 }, pool2 = { 0 }, pool3 = { 0 }, pool4 = { 0 }, pool5 = { 0 };
	//CDataBlob conv3norm = { 0 }, conv4norm = { 0 }, conv5norm = { 0 }, conv6norm = { 0 };
	CDataBlob conv3priorbox = { 0 }, conv4priorbox = { 0 }, conv5priorbox = { 0 }, conv6priorbox = { 0 };
	CDataBlob conv3priorbox_flat = { 0 }, conv4priorbox_flat = { 0 }, conv5priorbox_flat = { 0 }, conv6priorbox_flat = { 0 }, mbox_priorbox = { 0 };
	CDataBlob conv3loc_flat = { 0 }, conv4loc_flat = { 0 }, conv5loc_flat = { 0 }, conv6loc_flat = { 0 }, mbox_loc = { 0 };
	CDataBlob conv3conf_flat{ 0 }, conv4conf_flat = { 0 }, conv5conf_flat = { 0 }, conv6conf_flat = { 0 }, mbox_conf = { 0 };
	...
	cvector faces = cvector_create(sizeof(FaceRect));
	...
	total = 0.0;
	if (!setDataFrom3x3S2P1to1x1S1P0FromImage(&inputImage, rgbImageData, width, height, 3, step, param_pMean))
	{
		printf("setDataFrom3x3S2P1to1x1S1P0FromImage is failed\n");
	}
...
TIME_END("softmax")
	...
	dt = (t1.QuadPart - t0.QuadPart) / (double)nFreq.QuadPart;
	cout << "1 - 0:LastTIME = " << dt * 1000000 << endl;
	dt = (t2.QuadPart - t1.QuadPart) / (double)nFreq.QuadPart;
	cout << "2 - 1:LastTIME = " << dt * 1000000 << endl;
	dt = (t3.QuadPart - t2.QuadPart) / (double)nFreq.QuadPart;
	cout << "3 - 2:LastTIME = " << dt * 1000000 << endl;
	dt = (t4.QuadPart - t3.QuadPart) / (double)nFreq.QuadPart;
	cout << "4 - 3:LastTIME = " << dt * 1000000 << endl;
	dt = (t5.QuadPart - t4.QuadPart) / (double)nFreq.QuadPart;
	cout << "5 - 4:LastTIME = " << dt * 1000000 << endl;
	dt = (t6.QuadPart - t5.QuadPart) / (double)nFreq.QuadPart;
	cout << "6 - 5:LastTIME = " << dt * 1000000 << endl;
	dt = (t7.QuadPart - t6.QuadPart) / (double)nFreq.QuadPart;
	cout << "7 - 6:LastTIME = " << dt * 1000000 << endl;
	dt = (t8.QuadPart - t7.QuadPart) / (double)nFreq.QuadPart;
	cout << "8 - 7:LastTIME = " << dt * 1000000 << endl;
	dt = (t9.QuadPart - t8.QuadPart) / (double)nFreq.QuadPart;
	cout << "9 - 8:LastTIME = " << dt * 1000000 << endl;
	dt = (t10.QuadPart - t9.QuadPart) / (double)nFreq.QuadPart;
	cout << "10 - 9:LastTIME = " << dt * 1000000 << endl;
	dt = (t11.QuadPart - t10.QuadPart) / (double)nFreq.QuadPart;
	cout << "11 - 10:LastTIME = " << dt * 1000000 << endl;
	dt = (t12.QuadPart - t11.QuadPart) / (double)nFreq.QuadPart;
	cout << "12 - 11:LastTIME = " << dt * 1000000 << endl;

	CDataBlob facesInfo = {0};
	CDataBlobInit1(&facesInfo);
    TIME_START;
    ...
    {
        float score = getElementFloat(&facesInfo, i, 0, 0);
		float bbxmin = getElementFloat(&facesInfo, i, 0, 1);
		float bbymin = getElementFloat(&facesInfo, i, 0, 2);
		float bbxmax = getElementFloat(&facesInfo, i, 0, 3);
		float bbymax = getElementFloat(&facesInfo, i, 0, 4);
        FaceRect r;
        r.score = score;
		...
		cvector_pushback(faces, (void*)&r);
    }
    TIME_END("copy result");
    
    TIME_TOTAL(total);

	CDataBlobDeinit(&inputImage);
	for (i = 0; i < NUM_CONV_LAYER; ++i)
	{
		CDataBlobDeinit(&pConvDataBlobs[i]);
	}
	CDataBlobDeinit(&pool1);
	CDataBlobDeinit(&pool2);
	CDataBlobDeinit(&pool3);
	CDataBlobDeinit(&pool4);
	CDataBlobDeinit(&pool5);

	CDataBlobDeinit(&conv3priorbox);
	CDataBlobDeinit(&conv4priorbox);
	CDataBlobDeinit(&conv5priorbox);
	CDataBlobDeinit(&conv6priorbox);

	CDataBlobDeinit(&conv3priorbox_flat);
	CDataBlobDeinit(&conv4priorbox_flat);
	CDataBlobDeinit(&conv5priorbox_flat);
	CDataBlobDeinit(&conv6priorbox_flat);
	CDataBlobDeinit(&mbox_priorbox);

	CDataBlobDeinit(&conv3loc_flat);
	CDataBlobDeinit(&conv4loc_flat);
	CDataBlobDeinit(&conv5loc_flat);
	CDataBlobDeinit(&conv6loc_flat);
	CDataBlobDeinit(&mbox_loc);

	CDataBlobDeinit(&conv3conf_flat);
	CDataBlobDeinit(&conv4conf_flat);
	CDataBlobDeinit(&conv5conf_flat);
	CDataBlobDeinit(&conv6conf_flat);
	CDataBlobDeinit(&mbox_conf);
					 ...
					 int step) //input image, it must be RGB (three-channel) image!
{
    ...
    cvector faces = objectdetect_cnn(rgb_image_data, width, height, step);

    double t, total=0;
    TIME_START;

    int num_faces =(int)cvector_length(faces);
    ...
    for (i = 0; i < num_faces; i++)
    {
		...
		cvector_val_at(faces, i, &rect);
        p[0] = (short)rect.x;
		p[1] = (short)rect.y;
		p[2] = (short)rect.w;
		p[3] = (short)rect.h;
        p[4] = (short)(rect.score * rect.score * 100);
    }
